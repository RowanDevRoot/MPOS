...
function sharedPrefix(current, old, searchLength) {
  for (let i = 0; i < searchLength; i += 1) if (!equals(current[i], old[i])) return i;
  return searchLength;
}
...
function sharedSuffix(current, old, searchLength) {
  ...
  let count = 0;
  while (count < searchLength && equals(current[index1], old[index2])) index1 -= 1;
  index2 -= 1;
  count += 1;
...
function calcEditDistances(current, currentStart, currentEnd, old, oldStart, oldEnd) {
  ...
  // Initialize null row
  for (let j = 0; j < columnCount; j += 1) distances[0][j] = j;
