<!doctype html>
<!--
@license
Copyright (c) 2015 The Polymer Project Authors. All rights reserved.
This code may only be used under the BSD style license found at http://polymer.github.io/LICENSE.txt
The complete set of authors may be found at http://polymer.github.io/AUTHORS.txt
The complete set of contributors may be found at http://polymer.github.io/CONTRIBUTORS.txt
Code distributed by Google as part of the polymer project is also
subject to an additional IP rights grant found at http://polymer.github.io/PATENTS.txt
-->

<html>
<head>

  <title>cells-selector-basic</title>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0">

  <script src="../node_modules/@webcomponents/webcomponentsjs/webcomponents-loader.js"></script>
  <script src="../node_modules/wct-browser-legacy/browser.js"></script>

  <style>
    .cells-selected {
      background: #ccc;
    }

    .my-selected {
      background: red;
    }
  </style>

</head>
<body>

  <test-fixture id="defaults">
    <template>
      <cells-selector>
        <div>Item 0</div>
        <div>Item 1</div>
        <div>Item 2</div>
        <div>Item 3</div>
        <div>Item 4</div>
      </cells-selector>
    </template>
  </test-fixture>

  <br><br>

  <test-fixture id="basic">
    <template>
      <cells-selector selected="item2" attr-for-selected="id">
        <div id="item0">Item 0</div>
        <div id="item1">Item 1</div>
        <div id="item2">Item 2</div>
        <div id="item3">Item 3</div>
        <div id="item4">Item 4</div>
      </cells-selector>
    </template>
  </test-fixture>

  <script type="module">
    import '../cells-selector.js';

    suite('defaults', () => {
      let s1;

      setup(async () => {
        s1 = fixture('defaults');
        return await s1.updateComplete;
      });

      test('to nothing selected', () => {
        assert.equal(s1.selected, null);
      });

      test('to cells-selected as selectedClass', () => {
        assert.equal(s1.selectedClass, 'cells-selected');
      });

      test('to false as multi', () => {
        assert.isFalse(s1.multi);
      });

      test('to click as activateEvent', () => {
        assert.equal(s1.activateEvent, 'click');
      });

      test('to nothing as attrForSelected', () => {
        assert.equal(s1.attrForSelected, null);
      });

      test('as many items as children', () => {
        assert.equal(s1.items.length, s1.querySelectorAll('div').length);
      });
    });

    suite('basic', () => {
      var s2;

      setup(async () => {
        s2 = fixture('basic');
        return await s2.updateComplete;
      });

      test('honors the attrForSelected attribute', () => {
        assert.equal(s2.attrForSelected, 'id');
        assert.equal(s2.selected, 'item2');
        assert.equal(s2.selectedItem, document.querySelector('#item2'));
      });

      test('allows assignment to selected', async () => {
        s2.selected = 'item4';
        await s2.updateComplete;
        assert.isTrue(s2.children[4].classList.contains('cells-selected'));
        assert.equal(s2.selectedItem, s2.children[4]);
      });

      test('fire cells-select when selected is set', async () => {
        var selectedEventCounter = 0;
        s2.addEventListener('cells-select', (e) => {
          selectedEventCounter++;
        });
        s2.selected = 'item4';
        await s2.updateComplete;
        assert.equal(selectedEventCounter, 1);
      });

      test('set selected to old value', async () => {
        var selectedEventCounter = 0;
        s2.addEventListener('cells-select', (e) => {
          selectedEventCounter++;
        });
        s2.selected = 'item2';
        await s2.updateComplete;
        assert.equal(selectedEventCounter, 0);
      });

      test('force synchronous item update', () => {
        expect(s2.items.length).to.be.equal(5);
        s2.appendChild(document.createElement('div'));
        expect(s2.items.length).to.be.equal(5);
        s2.forceSynchronousItemUpdate();
        expect(s2.items.length).to.be.equal(6);
      });

      suite('`select()` and `selectIndex()`', () => {
        test('`select()` selects an item with the given value', async () => {
          s2.select('item1');
          await s2.updateComplete;
          assert.equal(s2.selected, 'item1');

          s2.select('item3');
          await s2.updateComplete;
          assert.equal(s2.selected, 'item3');

          s2.select('item2');
          await s2.updateComplete;
          assert.equal(s2.selected, 'item2');
        });

        test('`selectIndex()` selects an item with the given index', async () => {
          assert.equal(s2.selectedItem, s2.items[2]);

          s2.selectIndex(1);
          await s2.updateComplete;
          assert.equal(s2.selected, 'item1');
          assert.equal(s2.selectedItem, s2.items[1]);

          s2.selectIndex(3);
          await s2.updateComplete;
          assert.equal(s2.selected, 'item3');
          assert.equal(s2.selectedItem, s2.items[3]);

          s2.selectIndex(4);
          await s2.updateComplete;
          assert.equal(s2.selected, 'item4');
          assert.equal(s2.selectedItem, s2.items[4]);
        });
      });

      suite('items changing', () => {
        var s1;

        setup(async () => {
          s1 = fixture('defaults');
          return await s1.updateComplete;
        });

        test('cause cells-items-changed to fire', async () => {
          var newItem = document.createElement('div');
          var changeCount = 0;

          newItem.id = 'item999';

          s2.addEventListener('cells-items-changed', (event) => {
            changeCount++;
            var mutation = event.detail;
            assert.notEqual(mutation, undefined);
            assert.notEqual(mutation.addedNodes, undefined);
            assert.notEqual(mutation.removedNodes, undefined);
          });

          s2.appendChild(newItem);

          await s2.updateComplete;

          s2.removeChild(newItem);
          await s2.updateComplete;
          expect(changeCount).to.be.equal(2);
        });

        test('updates selected item', (done) => {
          s1.selected = 0;
          s1.updateComplete.then(() => {
            var firstElementChild = s1.children[0];
            expect(firstElementChild).to.be.equal(s1.selectedItem);
            expect(firstElementChild.classList.contains('cells-selected'))
                .to.be.eql(true);

            s1.addEventListener('cells-items-changed', () => {
              firstElementChild = s1.children[0];
              expect(firstElementChild).to.be.equal(s1.selectedItem);
              expect(firstElementChild.classList.contains('cells-selected'))
                  .to.be.eql(true);
              done();
            });

            s1.removeChild(s1.selectedItem);
          });

        });
      });

      suite('dynamic selector', () => {
        // NOTE(bicknellr): Polymer 2 only upgrades elements once they have been
        // connected to the document. This test now connects the selector
        // *first* and appends the child afterwards.
        test('selects dynamically added child automatically', (done) => {
          // Create the selector, set selected, connect to force upgrade.
          var selector = document.createElement('cells-selector');
          selector.selected = '0';
          document.body.appendChild(selector);

          // Create and append a new item which should become selected.
          var child = document.createElement('div');
          child.textContent = 'Item 0';
          selector.appendChild(child);

          selector.addEventListener(
              'cells-items-changed', function onCellsItemsChanged() {
                selector.removeEventListener(
                    'cells-items-changed', onCellsItemsChanged);

                assert.equal(child.className, 'cells-selected');
                document.body.removeChild(selector);
                done();
              });
        });
      });
    });
  </script>

</body>
</html>
