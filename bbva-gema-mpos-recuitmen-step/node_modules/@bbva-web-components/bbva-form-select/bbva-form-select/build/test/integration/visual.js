...
const screenshots = `${process.cwd()}/build/test/integration/screenshots`;
const currentDir = `${process.cwd()}/build/test/integration/screenshots/current`;
const baselineDir = `${process.cwd()}/build/test/integration/screenshots/baseline`;
...
describe('page screenshots are correct', function () {
  ...
  for (prefix in config.viewports) {
    ...
    describe(prefix, function () {
      ...
      const prefixTmp = prefix;
      beforeEach(async function () {
        return await page.setViewport(config.viewports[prefixTmp]);
      });
  
      for (const name of config.elements) {
        it(`/test/integration/${name}.html`, async function () {
          return await takeAndCompareScreenshot(page, name, prefixTmp);
        });
      }
    })
  }
...
async function takeAndCompareScreenshot(page, route, filePrefix) {
  ...
  await page.goto(`http://127.0.0.1:${port}/build/test/integration/${route}.html`, { waitUntil: ['load', 'networkidle0'] });
